...
/// <summary>
/// Real VRChat WebApi ported from the master branch's Dotnet/WebApi.cs, adapted to the
/// Tauri backend's storage/SQLite services. The HTTP handling is preserved as-is.
/// </summary>
public class WebApi
{
    ...
    public WebApi()
    {
        ...
        _timer = new System.Threading.Timer(TimerCallback, null, -1, -1);
    }
    ...
    private void TimerCallback(object state)
    {
        ...
        {
            Console.Error.WriteLine($"Failed to save cookies: {e.Message}");
        }
    }
    ...
    private void InitializeHttpClient()
    {
        ...
        {
            // Bound overall request time so a hung network call can't occupy
            // the single-threaded sidecar channel and stall all other IPC
            // (e.g. the database import) indefinitely.
    ...
    {
        var proxyUrl = Program.StorageGet("VRCX_ProxyServer");
        ...
        {
            Program.StorageSet("VRCX_ProxyServer", string.Empty);
            Console.Error.WriteLine("The proxy server URI you used is invalid.");
        }
    }
    ...
    private void LoadCookies()
    {
        ...
        var values = Sqlite.Execute("SELECT `value` FROM `cookies` WHERE `key` = @key",
        ...
        {
            Console.Error.WriteLine($"Failed to load cookies: {e.Message}");
        }
    }

    private List<Cookie> GetAllCookies()
    {
        var cookieTable = (System.Collections.Hashtable)CookieContainer.GetType().InvokeMember("m_domainTable",
    ...
    public void SetCookies(string cookies)
    {
        ...
        {
            Console.Error.WriteLine($"Failed to set cookies: {e.Message}");
        }
    ...
    private async Task<HttpRequestMessage> BuildLegacyImageUploadRequest(string url, IDictionary<string, object> options)
    {
        ...
        var fileToUpload = WebApiImageHelper.ResizeImageToFitLimits(Convert.FromBase64String(imageData), false);
    ...
    private async Task<HttpRequestMessage> BuildImageUploadRequest(string url, IDictionary<string, object> options)
    {
        ...
        var fileToUpload = WebApiImageHelper.ResizeImageToFitLimits(Convert.FromBase64String(imageData), matchingDimensions);
    ...
    private async Task<HttpRequestMessage> BuildPrintImageUploadRequest(string url, IDictionary<string, object> options)
    {
        if (options.TryGetValue("cropWhiteBorder", out var cropWhiteBorder) && (bool)cropWhiteBorder)
        {
            ...
            if (WebApiImageHelper.CropPrint(ref print))
            {
        ...
        var imageData = options["imageData"] as string;
        var fileToUpload = WebApiImageHelper.ResizePrintImage(Convert.FromBase64String(imageData));
    ...
    public async Task<Tuple<int, string>> Execute(IDictionary<string, object> options)
    {
        ...
        catch (HttpRequestException httpException)
        {
            if (httpException.InnerException != null)
                Console.Error.WriteLine($"{httpException.Message} | {httpException.InnerException}");
